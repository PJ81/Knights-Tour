...
class KnightTour {
  ...
  startHtml() {
    this.tourType = document.getElementById("knight").checked;
    if (this.tourType) {
      this.directions = [...this.knightDirs];
      this.knightPiece = "\u2658";
    } else {
      this.directions = [...this.kingDirs];
      this.knightPiece = "\u2654";
    }
  ...
  displayTour() {
    // plus one to take correct multiples of 10
    const digs = Math.ceil(Math.log10(this.cellCount * this.cellCount + 1));
    ...
    for (let x = 0; x < this.path.length; x += this.cellCount) {
      for (let y = x; y < x + this.cellCount; y++) {
        const z = this.path[y];
        const v = 1 + z.pos.x + z.pos.y * this.cellCount;
        str += v.toString().padStart(digs, "0") + " ";
      }
      str += "<br>";
    }
